...
const ShopHome: React.FC = () => {
  ...
  const nowMs = useMemo(() => {
    ...
    console.log(`[ShopHome] date de référence : "${dateRef}" → ${new Date(ms).toISOString()}`);
    return ms;
  }, [dateRef]);
  ...
  const filtered = useMemo(() => {
    ...
    if (badgeFilter === 'all') return bySearch;
    return bySearch.filter(p =>
      getProductBadge(p.date_availability_produit, nowMs) === badgeFilter.toUpperCase()
    );
  }, [products, search, badgeFilter, nowMs]);

  // DEBUG — retirer après vérification
  useMemo(() => {
    if (products.length > 0) {
      console.log('[ShopHome] produits chargés :', products.map(p => ({
        id: p.id,
        name: p.name,
        date_availability_produit: p.date_availability_produit ?? '(undefined)',
      })));
    }
  }, [products]);

  const hotCount = useMemo(() => products.filter(p => getProductBadge(p.date_availability_produit, nowMs) === 'HOT').length, [products, nowMs]);
  const newCount = useMemo(() => products.filter(p => getProductBadge(p.date_availability_produit, nowMs) === 'NEW').length, [products, nowMs]);
  ...
  return (
    <div className="space-y-16 pb-20 pt-10">
      ...
      <section className="flex flex-col gap-4">
        ...
        {/* Ligne 1 : recherche texte + filtres badges */}
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-2 text-xs font-semibold">
            {([
            ...
            ] as { key: BadgeFilter; label: string; count: number }[]).map(({ key, label, count }) => {
              ...
              return (
                ...
                >
                  {label}
                  {count > 0 && (
                    <span className={`ml-1.5 rounded-full px-1.5 py-0.5 text-[10px] ${active ? 'bg-white/20' : 'bg-slate-100 text-slate-400'}`}>
                      {count}
                    </span>
                  )}
                </button>
        ...
        {/* Ligne 2 : sélecteur de date de référence */}
        <div className={`flex flex-wrap items-center gap-3 rounded-2xl border px-4 py-3 text-sm transition-colors ${
          isSimulated ? 'border-amber-200 bg-amber-50' : 'border-slate-200 bg-white'
        }`}>
          ...
          <span className={`font-medium ${isSimulated ? 'text-amber-700' : 'text-slate-500'}`}>
            Date de référence
          </span>
          <input
            type="date"
            ...
            className={`rounded-lg border px-3 py-1.5 text-sm font-medium focus:outline-none focus:ring-2 ${
              isSimulated
                ? 'border-amber-300 bg-white text-amber-800 focus:ring-amber-300'
                : 'border-slate-200 bg-slate-50 text-slate-700 focus:ring-slate-300'
            }`}
          />
          {isSimulated && (
            <>
              <span className="text-xs text-amber-600">
                Simulation active — les badges HOT/NEW sont calculés à cette date
              </span>
              <button
                type="button"
                onClick={() => { setDateRef(''); setBadgeFilter('all'); }}
                className="ml-auto rounded-full border border-amber-300 bg-white px-3 py-1 text-xs font-semibold text-amber-700 transition hover:bg-amber-100"
              >
          ...
          {!isSimulated && (
            <span className="text-xs text-slate-400">
              Aujourd'hui · modifie la date pour simuler les badges à une autre époque
            </span>
          )}
        </div>
      </section>
            ...
            : filtered.length === 0
              ? (
                <div className="rounded-2xl border border-dashed border-slate-200 bg-white px-6 py-8 text-sm text-slate-500">
                  {badgeFilter === 'hot'
                    ? 'Aucun produit HOT en ce moment (sorti il y a moins de 24h).'
                    : badgeFilter === 'new'
                      ? 'Aucun produit NEW en ce moment (sorti il y a moins de 7 jours).'
